...
//	const char*  filename = "text_to_speech_test.wav";
	...
	int retval = zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ss", &filePath, &filePathLen, &toTransStr, &toTransStrLen);
	if(retval == FAILURE){
//		printf("iflytek: parse parameters fail\n");
	...
	//音频合成
	ret = text_to_speech(toTransStr,filePath,param);
	if ( ret != MSP_SUCCESS )
	{
